...
class language
{
	 ...
	 */
	public function __construct($loader, $common_modules = null)
	{
		$this->loader = $loader;
	 ...
	 * @param string|null	$tool_name		Name of the tool category to load component from, or null for core file
	 */
	public function add_lang($component, $tool_name = null, $phpbb_lang = false)
	{
		// Load common language files if they not loaded yet
		...
		if ($phpbb_lang == true)
		{
			...
			{
				$this->load_phpbb_file($component);
			}
			else
			{
				foreach ($component as $lang_file)
				{
					$this->add_lang($lang_file, null, true);
				}
			}
		}
		elseif (!is_array($component))
		{
			...
			{
				$this->load_tool_file('tools/' . $tool_name . '/' . $component);
			}
			else
			{
		...
		else
		{
			foreach ($component as $lang_file)
			{
				$this->add_lang($lang_file, $tool_name);
			}
		}
	}
	...
	{
		$number			= (int) $number;
		$plural_rule	= ($force_rule !== false) ? $force_rule : ((isset($this->lang['STK_PLURAL_RULE'])) ? $this->lang['STK_PLURAL_RULE'] : 1);

		if ($plural_rule > 15 || $plural_rule < 0)
		{
			throw new invalid_plural_rule_exception('STK_INVALID_PLURAL_RULE', array(
	 ...
	 * @param string	$component	Name of the component to load
	 */
	protected function load_phpbb_file($component)
	{
		// Check if the component is already loaded
		...
		$this->loader->load_phpbb($component, $this->language_fallback, $this->lang);
	 ...
	 *
	 * @param string	$component	path category/component to load
	 */
	protected function load_tool_file($component)
	{
		// Check if the component is already loaded
		if (isset($this->loaded_language_sets['tool'][$component]))
		{
			return;
		}

		$this->loader->load_tool($component, $this->language_fallback, $this->lang);
		$this->loaded_language_sets['tool'][$component] = true;
	}
	...
	protected function reload_language_files()
	{
		...
		// Reload phpbb files
		foreach ($loaded_files['phpbb'] as $component => $value)
		{
			$this->load_phpbb_file($component);
		}

		// Reload tool files
		foreach ($loaded_files['tool'] as $component => $value)
		{
			$this->load_tool_file($component);
		}
	}
}
