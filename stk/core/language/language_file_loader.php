...
use core\language\exception\language_file_not_found;
...
class language_file_loader
{
	 ...
	 */
	public function __construct($stk_root_path, $phpbb_root_path, $php_ext)
	{
	...
	public function load($component, $locale, &$lang)
	{
		...
		$path = $this->stk_root_path . 'language/';
	 ...
	 * @param array			$lang		Array reference containing language strings
	 */
	public function load_phpbb($component, $locale, &$lang)
	{
		$locale = (array) $locale;
		...
		$this->load_phpbb_file($path, $component, $locale, $lang);
	}

	/**
	 * Loads language array for the given tool component
	 *
	 * @param string		$tool	Name of the tool category
	 * @param string		$component	Name of the language component
	 * @param string|array	$locale		ISO code of the language to load, or array of ISO codes if you want to
	 * 									specify additional language fallback steps
	 * @param array			$lang		Array reference containing language strings
	 */
	public function load_tool($component, $locale, &$lang)
	{
		$locale = (array) $locale;
	...
	protected function load_file($path, $component, $locale, &$lang)
	{
		...
		if (strpos($this->stk_root_path . $component, $path) === 0)
		{
			// Filter out the path
			$path_diff = str_replace($path, '', dirname($this->stk_root_path . $component));
	 ...
	 * @param array		$lang		Array reference of language strings
	 */
	protected function load_phpbb_file($path, $component, $locale, &$lang)
	{
		// This is BC stuff and not the best idea as it makes language fallback
		// implementation quite hard like below.
