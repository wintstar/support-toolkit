...
use \core\stk_exception\stk_version_check_exception;

/**
 * Class to handle version checking and comparison
 */
class stk_version_helper
{
	/**
	 * @var string Host
	 */
	protected $host = 'stephan-frank.de';
	...
	/** @var $stk_config */
	protected $stk_config;

	/** @var \core\stk_file_downloader */
	protected $stk_file_downloader;
	 ...
	 */
	public function __construct(\phpbb\cache\service $cache, $stk_config)
	{
		$this->cache 				= $cache;
		$this->stk_file_downloader	= new stk_file_downloader();
		...
		$this->current_version = STK_VERSION;
	}
	 ...
	 * @param string $file File name (Default: stk.json)
	 * @param bool $use_ssl Use SSL or not (Default: false)
	 * @return version_helper
	 */
	public function set_file_location($host, $path, $file = 'stk.json', $use_ssl = false)
	{
		$this->host = $host;
		$this->path = $path;
		$this->file = $file;
	...
	* @return string
	* @throws stk_version_check_exception
	*/
	...
	public function get_ext_update_on_branch($force_update = false, $force_cache = false)
	{
		...
		preg_match('/^(\d+\.\d+).*$/', STK_VERSION, $matches);
	...
	*/
	...
	{
		$cache_file = '_stk_versioncheck_' . $this->host . $this->path . $this->file . $this->use_ssl;

		$info = $this->cache->get($cache_file);

		if ($info === false && $force_cache)
		{
			throw new \stk_exception\stk_version_check_exception('STK_VERSIONCHECK_FAIL');
		}
		else if ($info === false || $force_update)
		{
			$info = $this->stk_file_downloader->get($this->host, $this->path, $this->file, $this->use_ssl ? 443 : 80, 30);
			$error_string = $this->stk_file_downloader->get_error_string();

			if (!empty($error_string))
			{
				throw new \stk_exception\stk_version_check_exception($error_string);
			}
			...
			{
				throw new \stk_exception\stk_version_check_exception('VERSIONCHECK_FAIL');
			}
			...
			foreach ($versions_data as $branch => &$version_data)
			{
				...
				foreach ($version_data as $key => &$value)
				{
					if (!isset($this->version_schema[$stability_type][$key]))
					{
						...
						throw new \stk_exception\stk_version_check_exception('VERSIONCHECK_INVALID_ENTRY');
					}
					...
					switch ($this->version_schema[$stability_type][$key])
					{
						...
						case 'url':
							...
							{
								throw new \stk_exception\stk_version_check_exception('VERSIONCHECK_INVALID_URL');
							}
						break;
						...
						case 'version':
							...
							{
								throw new \stk_exception\stk_version_check_exception('VERSIONCHECK_INVALID_VERSION');
							}
						break;
